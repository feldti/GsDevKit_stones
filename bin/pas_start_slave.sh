...
if [ "$HOTSTANDBYBYENABLED" = "true" ]; then
  nohup $GEMSTONE/bin/startlogreceiver -P $HOTSTANDBYMASTERPORT -A $HOTSTANDBYMASTERADR  -T $HOTSTANDBYSOURCE -s $PAS_STONE_NAME -l $GEMSTONE_LOGDIR &
  echo "LOGRECEIVER started"
fi
#
#
#
cat << EOF | $GEMSTONE/bin/topaz -lq -T 1000000 -u pum_continous_restore
set user DataCurator pass $GEMSTONE_CURATOR_PASS gems $stoneName
iferror where
login
run
SystemRepository continuousRestoreFromArchiveLogs:{'$HOTSTANDBYSOURCE'}
%
EOF
